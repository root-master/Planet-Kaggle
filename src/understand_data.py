...
def sample_to_fname_tif(sample_df, row_idx, suffix='tif'):
...
def plot_rgbn_histo_tif(r, g, b, n):
...
def sample_to_fname_jpg(sample_df, row_idx, suffix='jpg'):
    '''Given a dataframe of sampled images, get the
    corresponding filename.'''
    fname = sample_df.get_value(sample_df.index[row_idx], 'image_name')
    return '{}.{}'.format(fname, suffix)
...
#bgr_image = bgrn_image[:,:,:3] # ORIGINAL 

bgr_image = bgrn_image[:,:,:]
...
for i in range(len(sampled_images)):
    ...
    jpg = sample_to_fname_jpg(sampled_images, i, 'jpg')
